...
if [ "${deploy}" == "true" ]; then
   ...
   if [ -n "${profile}" ]; then
      ...
      echo yes | mvn site site:deploy -P "${profile}" -Denforcer.skip=true --settings ~/settings.xml
   else
      echo yes | mvn site site:deploy -Denforcer.skip=true --settings ~/settings.xml
   fi
